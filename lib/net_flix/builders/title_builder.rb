class TitleBuilder
  ...
  def set_id
    ...
    if node and node.content[/[0-9]*$/]
      @title.id = node.content[/[0-9]*$/]
    end
  end

  def set_url
    node = @data.search('id').first
    @title.url = node.content if node
  end

  def set_average_rating
    node = @data.search('average_rating').first
    @title.average_rating = node.content if node
  end
  ...
  def set_actors
    @title.actors = PeopleBuilder.from_movie(@data)
  end

  def set_directors
    @title.directors = PeopleBuilder.from_movie(@data, :directors)
  end
